...
from .helpers.defaulters import add_default_bets, is_default_bets_present
...
class MatchUpdateSerializer(serializers.ModelSerializer):
    ...
    bfirst = serializers.CharField(write_only=True)

    winner = TeamSerializer(read_only=True)
    wTeam = serializers.CharField(write_only=True)
    ...
    def validate(self, attrs):
        ...
        if winner and self.instance.status != settings.SCHEDULED:
            raise serializers.ValidationError(
